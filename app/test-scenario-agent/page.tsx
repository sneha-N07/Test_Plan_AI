...
export default function TestScenarioAgentPage() {
  ...
  const generateTestScenarios = async () => {
    try {
      setLoading(true);
      setTestScenarios(''); 
      setStep(2); // Now the Test Scenarios step
  ...
  return (
    <div>
      ...
      <div className="stepper glass-panel" style={{ padding: '0.5rem', marginBottom: '20px' }}>
        {[1,2].map(s => {
          const isClickable = s === 1 || (s === 2 && testScenarios !== '');
          return (
            <div 
              key={s} 
            ...
            >
              {s}. {['Ticket Information', 'Test Scenarios'][s-1]}
            </div>
          );
        })}
      </div>
      ...
      {step === 1 && (
        <div className="glass-panel">
          ...
          <div style={{ marginTop: '2rem', display: 'flex', gap: '1rem' }}>
            <button className="btn-primary" style={{ width: '100%' }} onClick={generateTestScenarios} disabled={(!jiraTicketId && !context) || loading}>
