...
import { Target, RotateCcw, Download, Settings, Loader2, Briefcase, FileText } from 'lucide-react';

export default function TestCaseAgentPage() {
  ...
  useEffect(() => {
    ...
    // Load history
    setJiraTicketId(localStorage.getItem('tc_jiraTicketId') || '');
    setContext(localStorage.getItem('tc_context') || '');
    setTestCases(localStorage.getItem('tc_testCases') || '');
    const savedStep = localStorage.getItem('tc_currentStep');
    if (savedStep) setStep(parseInt(savedStep));
  }, []);

  // Persist history automatically
  useEffect(() => {
    localStorage.setItem('tc_jiraTicketId', jiraTicketId);
    localStorage.setItem('tc_context', context);
    localStorage.setItem('tc_testCases', testCases);
    localStorage.setItem('tc_currentStep', step.toString());
  }, [jiraTicketId, context, testCases, step]);
    ...
    try {
      ...
      if (jiraTicketId.trim() !== '') {
        ...
        if (!fetchData.success || !fetchData.issues || fetchData.issues.length === 0) {
          setTestCases("Error: Could not fetch Jira ticket information. " + (fetchData.error || "Ticket not found."));
          setLoading(false);
          return;
        }
        ...
        if (!context || context.trim() === '') {
          setTestCases("Error: You must provide a Jira ticket ID or a description/context.");
          setLoading(false);
          return;
        }
      ...
      const genRes = await fetch('/api/llm/generate-test-cases', {
      ...
      if (genData.success) {
        ...
        // Optionally update history if you want test cases history
        try {
          const historyRaw = localStorage.getItem('testPlanHistory');
          const history = historyRaw ? JSON.parse(historyRaw) : [];
          const newEntry = {
            id: Date.now().toString(),
            ticketId: jiraTicketId || 'Custom Desc',
            date: new Date().toLocaleString(),
            plan: newCases, // we use 'plan' to reuse the history modal
        ...
        setTestCases("Error generating test cases: " + genData.error);
      }
    } catch (err: any) {
      ...
      {step === 3 && (
        <div className="glass-panel">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            ...
            {testCases && <button className="btn-secondary" style={{ display: 'flex', gap: '8px' }} onClick={() => {
              const blob = new Blob([testCases], { type: 'text/csv' });
          ...
          {loading && (
            <div style={{ textAlign: 'center', padding: '4rem', color: '#10b981' }}>
               ...
               <p style={{ marginTop: '1rem' }}>AI is crafting your test cases...</p>
            </div>
          )}
